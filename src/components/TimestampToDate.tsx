  ...
  const convertToDate = () => {
    ...
    try {
      ...
      if (ts.toString().length === 13) {
        // Milliseconds
        ts = ts;
      } else if (ts.toString().length === 16) {
        // Microseconds
        ts = ts / 1000;
      } else if (ts.toString().length === 19) {
        // Nanoseconds
        ts = ts / 1000000;
      } else {
        // Seconds
      ...
      <CardContent className="space-y-4">
        <div className="space-y-2">
          ...
          <Input
            id="timestamp"
            type="text"
            placeholder="1763384855"
            value={timestamp}
            onChange={(e) => setTimestamp(e.target.value)}
            onKeyDown={(e) => e.key === "Enter" && convertToDate()}
          />
        </div>
        ...
        <div className="flex gap-2">
          <Button onClick={convertToDate} className="flex-1">
